# CinéTrack — Chapter 25 Snapshot

Snapshot of CinéTrack at the end of Chapter 25: *Security, Secrets, and What NEVER to Send to AI*.

## What changed in this chapter

Security review identified an authorization gap: `DELETE /reviews/{id}` allowed any authenticated user to delete any review. Fixed with ownership enforcement:

- **`ReviewService.delete(Long id, String requestingUsername)`** — rejects delete if the review's author username doesn't match the authenticated principal
- **`ReviewController.delete()`** — injects Spring Security `Authentication`, passes `authentication.getName()` to the service
