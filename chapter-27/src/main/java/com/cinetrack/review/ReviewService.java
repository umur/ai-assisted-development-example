...
class ReviewService {
    ...
    @Transactional(readOnly = true)
    public Optional<Page<ReviewResponse>> findByAuthor(Long userId, Pageable pageable) {
        if (!users.existsById(userId)) {
            return Optional.empty();
        }
        return Optional.of(reviews.findByAuthorId(userId, pageable).map(ReviewMapper::toResponse));
    }
    ...
    @Transactional
    public boolean delete(Long id, String requestingUsername) {
        return reviews.findById(id).map(review -> {
            if (!review.getAuthor().getUsername().equals(requestingUsername)) {
                return false;
            }
